...
function template_arcade_list()
{
	...
	/*  javascript for game popup  */
	echo '
<script type="text/javascript"><!--
	var WindowObjectReference = null;
	function myGamePopupx(strURL,strWidth,strHeight)
	{
		var strOptions="";
		var strType = "console";
...
//--></script>';
	...
	foreach ($context['arcade']['games'] as $game)
	{
		...
		if ($context['arcade']['can_favorite'])
		{
			...
			if (!$game['is_favorite'])
				$fav .= '
							<img id="favgame' . $game['id'] . '" src="' . $settings['images_url'] . '/favorite.gif" height="16" width="16" alt="' . $txt['arcade_add_favorites'] . '" />' . '
						</a>';
			else
				$fav .= '
							<img id="favgame' . $game['id'] . '" src="' . $settings['images_url'] . '/favorite2.gif" height="16" width="16" alt="' . $txt['arcade_remove_favorite'] . '" />
							</a>';
		}
		...
		if (empty($game['pdl_count'])) {$game['pdl_count'] = 0;}
			$game['height'] = $game['height'] + 20;

		$pop = '
							<a href="javascript:void(0)" onclick="myGamePopupx(\'' . $game['url']['play'].';pop=1'.'\',\'' . $game['width'] . '\',\'' . $game['height'].'\')">' . $txt['pdl_popplay'] . '</a>';
		...
		if ((AllowedTo('arcade_admin') == true) && ((int)$game['report_id'] > 0))
			$viewreport .= '<br /><b>&bull;</b>&nbsp;' . $txt['show_pdl_report'];

		if ($modSettings['arcadeEnableDownload'] == true)
			$viewdl .= '
							<b>&bull;</b>&nbsp;'. $txt['pdl_counter']. '&nbsp;' .$game['pdl_count'].'<br />
							<b>&bull;</b>&nbsp;
							<a href="' . $scripturl.'?action=arcade;sa=download;game=' . $game['id'] . '">' . $txt['arcade_download_game'] . '</a><br />';
		...
		$code .= $open . '
							<div class="titlebg" style="height: 18px;padding:2px 5px 2px 5px;margin:2px 5px 2px 5px;border-bottom:1px solid #808080">
								<div class="titlebg" style="float: left;padding-top: 1px;" >
									<a href="' . $game['url']['play'] . '" title="' . $txt['alt_play'] . '">' . $game['name'] . ' </a>
								</div>
